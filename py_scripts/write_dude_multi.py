...
def read_pdb(path):
    ...
    residue_name = pdb_df.df["ATOM"]["chain_id"] + pdb_df.df["ATOM"][
        "residue_number"
    ].astype(str)
...
def read_sdf_gz_3d(path):
    ...
    ms = [
        rdMolStandardize.Uncharger().uncharge(Chem.RemoveHs(m))
        for m in ms
        if m is not None
    ]
    return ms
...
def parser(protein_path, mol_path, ligand_path, activity, pocket_index, raid=6):
    ...
    pocket_atom_idx = [
        i for i, r in enumerate(protein["residue_name"]) if r in pocket_residue
    ]
...
if __name__ == "__main__":
    protein_path = [
        os.path.join(args.mol_data_path, x, "receptor.pdb")
        for x in os.listdir(args.mol_data_path)
    ]
    act_mol_path = [
        os.path.join(args.mol_data_path, x, "actives_final.ism")
        for x in os.listdir(args.mol_data_path)
    ]
    decoy_mol_path = [
        os.path.join(args.mol_data_path, x, "decoys_final.ism")
        for x in os.listdir(args.mol_data_path)
    ]
    ...
    for i, pocket in tqdm.tqdm(enumerate(protein_path)):
        ...
        data = []
        d_active = mol_parser(
            act_mol_path[i], pocket.replace("receptor.pdb", "crystal_ligand.mol2"), 1
        )
        ...
        # decoy mols
        d_decoy = mol_parser(
            decoy_mol_path[i], pocket.replace("receptor.pdb", "crystal_ligand.mol2"), 0
        )
        ...
        # write pocket
        d = pocket_parser(
            pocket, pocket.replace("receptor.pdb", "crystal_ligand.mol2"), i
        )
